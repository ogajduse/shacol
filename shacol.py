...
class Shacol(object):
    ...
    def findCollisionIntensity(self, hashPart=None):
        ...
        try:
            ...
            #bloomFilter = pybloomfilter.BloomFilter(1000000000, 0.01) #up to 10 bilions
            bloomFilter = pybloof.StringBloomFilter(size=100000000,hashes=9)
            start = timeit.default_timer()
            ...
            while newHashPart not in bloomFilter:
                ...
                if status == 10000000:
                    print('\n' * 100)
                    print('Count of cycles:', count)
                    print('Run time:', round((timeit.default_timer() - start) / 60, 3))
                    status = 0
            ...
            print('Time:', round((timeit.default_timer() - start), 3))
            ...
            while newHashPart != collisionHash:
                ...
                if status == 10000000:
                    ...
                    print('Count of cycles:', count)
                    print('Run time:', round((timeit.default_timer() - start), 3))
                    status = 0
                strHashPart = binascii.unhexlify(newHashPart)
                newHash = hashlib.sha256(strHashPart).hexdigest()
                newHash = newHash[0:hashPartLength]
            ...
            print('Time consumption of collision check:', round((timeit.default_timer() - start2), 3))
            stop = timeit.default_timer()
            totalTime = round(stop - start, 12)
            ...
            print('\nInput hashPart:', hashPart)
            print("Collision found after %s seconds" % (totalTime))
            print('Count of the cycles:', index)
            print('Collision hash:', newHash)
            print('Cycles between collision hashes:', index-count)

            return {"inputHash": hashPart, "time": totalTime, "cycles": index, "collisionHash": newHash,
                    "indexOfCollision": index, "cyclesBetCol": index-count}
    ...
    def findCollisionBloom2(self, hashPart=None):
        """
        The test method using Bloom filter based on mmap.
        ...
        try:
            ...
            newHashPart = bytes(hashPart, 'utf-8')
            bloomFilter = pybloomfilter.BloomFilter(100000000, 0.01)#, '/tmp/words.bloom') #without last argument = in memory
            start = timeit.default_timer()
